...
class Submitter extends _rx.Subject {
  ...
  submit(messages) {
    ...
    return _asyncToGenerator(function* () {
      ...
      console.log(JSON.stringify(messages)); // eslint-disable-next-line no-buffer-constructor

      const payloadText = new Buffer(JSON.stringify(messages), "utf8");
      const dataListBuffer = yield _this.gzip ? _zlib.default.gzip(payloadText) : payloadText;
      ...
      const headers = {
        ...
        "Dry-Run": _this.dryRun ? "true" : undefined
      };
      debug("Post to %s", _this.url);
      debug("Headers: %o", headers);
      debug("Body: %o", body);
      debug("Posting...");
      const response = yield (0, _nodeFetch.default)(_this.url, {
        method: "POST",
        headers,
        body,
      ...
      if (_this.debug && messages.count > 1 && response.status === 400) {
