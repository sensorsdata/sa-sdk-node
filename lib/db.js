...
let DBCache = /*#__PURE__*/function () {
  ...
  _createClass(DBCache, [{
    ...
    key: "uploadCache",
    value: function () {
      var _uploadCache = _asyncToGenerator(function* (upload) {
        this.selectAll().then(events => {
          events.forEach(event => {
            //   this.deleteById(event);
            upload(event); //   const message = JSON.parse(event.message);
