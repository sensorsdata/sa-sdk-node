...
class TaskQueue {
  ...
  executeTask() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const data = _this.dequeue();

      try {
        debug("Consume data: %o", data);
        const result = yield _this.consumeData(data);
        debug("Succeeded");

        if (_this.onSucceeded != null) {
          _this.onSucceeded(result);
        }
      } catch (ex) {
        debug("Failed: %s", ex);
      ...
      return _this.executeTask(); // No await to flattern cascaded promises
    })();
  }
